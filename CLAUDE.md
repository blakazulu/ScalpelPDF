...
## Release & distribution facts

- **Releasing anything → follow [`docs/RELEASING.md`](docs/RELEASING.md)** (the canonical runbook) or invoke the **`/release`** skill (`.claude/skills/release/SKILL.md`), which wraps it step by step.
- **Store listing:** <https://apps.microsoft.com/detail/9n9hn8xw4lf3> · **Website:** <https://scalpel-pdf.netlify.app>.
- The website **auto-deploys via Netlify on every push to GitHub `main`** (base directory `website/`). It hosts no binaries — `/download` and `/installer` redirect to the latest GitHub Release assets (`Scalpel.exe` / `Scalpel-Setup.exe`), and `website/public/version.json` is the in-app update-check signal, so deploy it (i.e. push) **last** in a release.
- **Publisher identity:** `Liraz Shaka Amir` (Partner Center `PublisherDisplayName`; verify against the Product identity page before every Store upload — a one-character mismatch rejects the package).
- **Never use the `msstore` CLI** — it crashes/hangs on this machine. Store uploads are manual in the Partner Center website.
- **Inno Setup is installed per-user, NOT in Program Files:** `C:\Users\Liraz\AppData\Local\Programs\Inno Setup 6\ISCC.exe` (`installer\build-installer.ps1` checks this path first).
- **Pre-push hook** (`build/hooks/pre-push`, wired via `core.hooksPath`): every push to `main` auto-bumps the 4th (revision) version component in `Scalpel.csproj`, commits it, and **aborts the first push by design** — just run `git push` again. Real releases set Major.Minor.Build by hand (see RELEASING.md §1).

## Architecture

WPF desktop app. There is no MVVM framework in play for the main window — it is a large code-behind class.

- **`MainWindow` is one `partial class` split across ~30 `MainWindow.<Area>.cs` files.** The ~9,700-line monolith was split (no behavior change) into focused partials named by responsibility: `Settings`, `WindowChrome`, `ContextMenu`, `FileOps`, `Links`, `Forms`, `AppMode`, `ToolSelection`, `Outline`, `Crop`, `DrawBar`, `TextBar`, `Signatures`, `CanvasInteraction`, `Selection`, `Search`, `InlineTextEdit`, `TextBox`, `KeyboardShortcuts`, `AnnotationManagement`, `DirtyTracking`, `FileToolbar`, `SaveAnnotations`, `TempReload`, `Zoom`, `DragDrop`, `PageSelection`, `ViewMode`, `Dialogs`, `ExportImages`, `Compare`, `Measure`, `Straighten`, `FormOcr` (this last holds the standalone `ScalpelDialog` MessageBox replacement, not a `MainWindow` member), `Tabs` (the document-tab strip and every open/switch/close code path - see the **Tabs** section below), `DocumentSession` (the per-tab state container), plus the pre-existing `Tools` and `Update`. The slim core **`MainWindow.xaml.cs`** (~310 lines) keeps only the field declarations, the constructor, and the window-init/maximize (`WM_GETMINMAXINFO`) plumbing. When adding UI behavior, find or add the matching area file rather than growing the core. `MainWindow.xaml` is the matching layout. Sections are still marked with `// ====` banner comments. (The upstream `docs/github-origin` KillerPDF dump is reference-only and excluded from compilation in `Scalpel.csproj`; it is untracked and currently holds the **v1.8.1** tree (net10 + KillerPdf.Engine, refreshed 2026-08-30) - see `docs/KILLERPDF-UPSTREAM-1.8.1-ANALYSIS.md` for everything upstream changed since the v1.6.0 fork point.)
...
- **Tabs (Chrome-style document tabs, `MainWindow.Tabs.cs`).** Every open PDF is its own live `DocumentSession` held in a `TabListModel<DocumentSession>` (`Services/TabListModel.cs`); `_s` is whichever session is on screen. Switching tabs never reopens the file - it is `DeactivateSession()` on the outgoing session, then `_s = target`, then `BindSessionToUi(target, newContent: false)` - so each tab keeps its own unsaved annotations, undo/redo, dirty flag, zoom, page and scroll position across a switch. The tab strip (option-C compact chips, always shown under the ribbon band) is rebuilt from the session list by `RefreshTabStrip`; opening/closing/reordering all go through `OpenInTab`/`OpenManyInTabs`, `CloseSession`/`CloseMany`, and `_tabs.Move`. A `LongOperationGate` (`Services/LongOperationGate.cs`), held as `_longOps`, is taken for the duration of every long document operation (compress, OCR, redact, straighten, form OCR, flatten, print, export images, compare); while `_longOps.IsBusy`, tab switching, closing and the window close are all refused with the `Str_Tab_Busy` toast, so a result can never land in a document other than the one it started on. Each tab's own working/undo/redo/burn temp files are made with `App.MakeTempFile(tag, owner)` keyed by the session's `Id` and deleted immediately on close via `App.ReleaseTempFiles(owner)`, rather than waiting for the exit sweep. At startup, `TryRestoreOpenTabs()` reopens the tabs open when Scalpel last closed (the `OpenTabs` registry setting, written by `SaveOpenTabsSetting`): the tab that was active loads its document right away, and every other restored tab becomes a **deferred** tab (a chip with a real name but no loaded document) that loads through the normal open pipeline the first time it is activated.
- **`App.xaml.cs`** is the entry point and orchestrator. It handles crash dialogs (3 unhandled-exception sinks), delegates install/uninstall to `Services/Installer.cs` and `Services/InstallerUI.cs`, and manages the pdfium integrity check, settings, and temp-file lifecycle. It is also **single-instance** (`Services/SingleInstance.cs`): a per-user-session mutex picks the primary; any later launch forwards its command line over a named pipe and exits, and the primary opens each forwarded file as its own tab (live `DocumentSession`) via `MainWindow.HandleForwardedLaunch` (`MainWindow.SingleInstance.cs`) and comes to the front. If the primary does not answer within 2 s the new process just runs normally. `SCALPEL_MULTI_INSTANCE=1` opts out - the E2E harness sets it because it runs several instances in parallel.
...
- **PdfSharpCore** — reading/writing PDF document **structure**: opening (`PdfReader.Open`), merge/split, page ops, and burning annotations on save. Note: it can *read* encrypted PDFs but cannot *re-save* them once modified, so the code decrypts to a temp copy first. **Gotcha:** never read `doc.Outlines` on a document unless `doc.Internals.Catalog.Elements.ContainsKey("/Outlines")` - on a bookmark-less PDF the read creates an empty outline object that PdfSharpCore then never serialises, and the saved file gets a dangling xref entry PdfSharpCore itself cannot reopen ("Unexpected token 'xref'"). `Services/PdfSaveGuard.PrepareForSave(doc)` strips such an entry before user-facing saves, and `Services/PdfReopen.OpenModify` recovers files that already have it via a PDFium re-save.
...
The Tools dropdown (right of the ribbon tab strip; handlers `ToolsNumbering/Compress/Ocr/Redact/Protect/Sanitize_Click`) runs document-level operations through standalone, mostly WPF-free services rather than the `MainWindow` monolith: `BatesNumberingService` (page/Bates numbers + corner headers/footers), `PdfCompressionService` (image downsampling, Low/Med/High), `RedactionService`, `PdfEncryptionService` (password + permissions), `MetadataSanitizer`, `PreflightService`, `PdfComments` (read **and edit** the notes reviewers left; `CommentsWindow` is the editor), `PdfPageDifference` (compare two files), `MeasurementCalculator` (measure), `PerspectiveWarp` (straighten a photographed page), `FormOcrPolicy` (fill fields by OCR), `PageQualityConverter` (export pages as images), and OCR (`OcrService` + `TesseractCliOcrEngine`, page rasterization via `PdfRasterTools` / `DocnetPageRasterizer`).
...
- **Temp files** → `scalpel_p<pid>_<tag>_<guid>.pdf` under `%LOCALAPPDATA%\Scalpel\Temp`, registered per-session and swept on exit (`CleanupSessionTemps`). The startup sweep (`CleanupStaleTemps`) is **PID-aware** via `Services/TempSweep.cs`: it only deletes files whose owning Scalpel process is gone. Never revert it to a blind `scalpel_*.pdf` wipe - decrypted/repaired/network-copied documents live in that folder, and a second instance starting up used to delete the first window's working copy ("could not open file" mid-session).
...
### PDFium is single-threaded - always go through `PdfiumGate`

`Services/PdfiumGate.cs` owns one dedicated thread and **every** PDFium/Docnet call runs on it
(`PdfiumGate.Run(...)`). PDFium keeps global state and Docnet stands up a form-fill environment per
document; using it from more than one thread produces an `AccessViolationException` inside
`FPDFDOC_ExitFormFillEnvironment`, which .NET Framework will not let the app catch (`App.xaml.cs`
deliberately omits `HandleProcessCorruptedStateExceptions`) - so it kills the process with no crash
dialog and nothing in the log. When adding a render path, marshal the **native calls only**, never a
whole loop: the streaming loops call `Dispatcher.Invoke` between pages, and running that on the
PDFium thread would block it on the UI while the UI waits on PDFium. Never abandon a reader either -
a reader left to the GC finalizer is torn down off-thread. **Open readers with
`Services/PinnedDocReader.Open(...)`, never `DocLib.Instance.GetDocReader(...)`:** Docnet 2.6.0 hands
PDFium a managed `FPDF_FORMFILLINFO` that PDFium keeps a pointer to and reads again at teardown; a GC
in between moves it and `FPDFDOC_ExitFormFillEnvironment` jumps through garbage (the same uncatchable
AV, even on the PDFium thread). `PinnedDocReader` pins that block for the reader's life. `Scalpel.Tests` disables xUnit
parallelization for the same reason (`AssemblyInfo.Tests.cs`); do not re-enable it.

### Canvas/PDF coordinates live in one place

`Services/PageSpaceMap.cs` converts rectangles between the rendered canvas (y-down, rotation already
applied) and PDF user space (y-up, unrotated). The crop box and the form-widget overlay both use it.
Do not hand-roll this again: the two call sites previously derived it separately and disagreed at
180 degrees, where the rotation's vertical flip and the y-up/y-down flip cancel out, so y passes
through unchanged. Crop applied the flip anyway and wrote a `/CropBox` for the mirrored half of the
page - and because its forward and inverse copies were wrong the same way, the round trip agreed
with itself and the overlay looked correct.
...
- `docs/KILLERPDF-UPSTREAM-1.8.1-ANALYSIS.md` - inventory of every fix/feature upstream KillerPDF shipped between v1.6.0 (fork point) and v1.8.1, with issue numbers and a transferable / not-transferable split for the port program.
